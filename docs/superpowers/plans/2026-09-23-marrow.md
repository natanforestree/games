...
```
.gitignore / art/marrow/.gitignore     previews and the style-test composite aren't committed
art/marrow/
...
```bash
cd /Users/nathan/Documents/code/games && git switch -c marrow
```
...
export function findLedge(f, screen, side, range, open = CLOSED) {
  ...
  for (let row = Math.floor((head - range) / TILE); row <= Math.floor((head + range) / TILE); row++) {
    ...
    if (room) return { side, col, top, cornerX };
  }
  return null;
}
...
function fallSword(s, screen) {
  ...
  const ny = s.y + s.vy, row = Math.floor(ny / TILE);
  if (s.vy > 0 && P.solidAt(screen, Math.floor(s.x / TILE), row)) {
...
```bash
cd /Users/nathan/Documents/code/games
git add marrow/src/combat.js marrow/src/fighter.js marrow/test/combat-unarmed.test.js
...
4. Check `browser_console_messages`: the only error should be the one from the deliberate failure.

- [ ] **Step 8: Commit**

```bash
cd /Users/nathan/Documents/code/games
git add art/marrow/poses.lua art/marrow/fighter.aseprite marrow/assets/fighter.png marrow/assets/fighter.json marrow/src/assets.js marrow/src/draw-fighters.js marrow/src/render.js marrow/src/main.js marrow/test/draw-fighters.test.js marrow/test/assets.test.js
...
async function boot() {
  try {
    const [assets] = await Promise.all([loadAssets(), document.fonts.load('8px Silkscreen')]);
...
In `index.html`, add this card directly after the Snake card (the `<a class="card" href="./snake/">…</a>` block):
```html
...
```bash
cd /Users/nathan/Documents/code/games
git add art/marrow/icon.lua art/marrow/icon.aseprite marrow/icon.png index.html README.md
git commit -m "Marrow: card icon, games-page card and README"
```

- [ ] **Step 5: Publish, with Nathan's OK**

Ask Nathan before pushing. Pushing publishes Marrow on the live site. The design spec and this plan under `docs/` would become public too. Ask whether that's fine; if not, remove `docs/` from the repo on this branch before merging.
